((E, $) ->
  ...
  $(document).ready (event) ->
    # I'm so sorry about it : THIS IS NOT CLEAN, but I haven't found better repair.
    $("a[href$='/planning/load_plans']").attr('target', '_self')

    return unless window.location.pathname == '/planning/load_plans'
    if time == 0
      time++
      ...
      planChargeChart = new E.PlanChargeChart()
  ...
  class PlanChargeChart
    ...
    _loadCharges: (datas, callback) ->
      ...
      $.ajax
        url: '/planning/load_plans/period_charges'
        data: datas
    ...
    _displayDetails: (datas, ajaxify) ->
      ...
      if ajaxify
        $("#bar-chart").parent().append('<i class="picto picto-circle-o-notch picto-spin"></i>')
        $.ajax
          url: '/planning/load_plans/period_charges_details'
          data: datas
          success: (data, status, request) =>
